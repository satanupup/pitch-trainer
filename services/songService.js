...
async function getSongs() {
    const [rows] = await dbPool.query('SELECT name, mp3_path, vocal_path, midi_path, lrc_path FROM songs ORDER BY created_at DESC');
    return rows.map(row => ({ 
        name: row.name, 
        mp3: row.mp3_path, 
        vocal: row.vocal_path,
        midi: row.midi_path, 
        lrc: row.lrc_path 
    }));
}
...
async function getJobStatus(jobId) {
    const [rows] = await dbPool.query(`
        SELECT j.*, s.name, s.mp3_path, s.vocal_path, s.midi_path, s.lrc_path 
        FROM jobs j 
